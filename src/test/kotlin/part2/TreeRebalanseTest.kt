...
class TreeRebalanseTest {
    ...
    fun `левый поворот`() {
        ...
        assertEquals(2, getNodeData(root), "Корень должен быть 2 после ребалансировки")
        assertFalse(isNodeRed(root!!), "Корень должен быть черным")

        val leftChild = getNodeLeft(root)
        assertNotNull(leftChild, "Левый потомок не должен быть null")
        assertEquals(1, getNodeData(leftChild), "Левый потомок должен быть 1")
        assertTrue(isNodeRed(leftChild!!), "Левый потомок должен быть красным")

        val rightChild = getNodeRight(root)
        assertNotNull(rightChild, "Правый потомок не должен быть null")
        assertEquals(3, getNodeData(rightChild), "Правый потомок должен быть 3")
        assertTrue(isNodeRed(rightChild!!), "Правый потомок должен быть красным")
    }
    ...
    fun `смена цвета узла`() {
        ...
        val root = getRoot(tree)
        assertNotNull(root, "Корень не должен быть null")
        assertEquals(2, getNodeData(root), "Корень должен быть 2")
        assertFalse(isNodeRed(root!!), "Корень должен быть черным после изменения цвета")
    ...
    @Test
    fun `множественные ребалансировки`() {
        val tree = RedBlackTree<Int>()
        ...
        assertEquals(insertSequence.size, tree.size, "Размер должен соответствовать количеству вставленных элементов")

        // Проверяем, что порядок сохраняется
        assertEquals(insertSequence.sorted(), tree.toList(), "Элементы должны быть в отсортированном порядке")
    }

    @Test
    fun `левый поворот в узле`() {
        val tree = RedBlackTree<Int>()
        ...
        val root = getRoot(tree)
        assertNotNull(root, "Корень не должен быть null")
        assertEquals(2, getNodeData(root), "Корень должен быть 2 после лево-правого поворота")
        assertFalse(isNodeRed(root!!), "Корень должен быть черным")
    ...
    fun `правый поворот в узле`() {
        ...
        val root = getRoot(tree)
        assertNotNull(root, "Корень не должен быть null")
        assertEquals(2, getNodeData(root), "Корень должен быть 2 после право-левого поворота")
        assertFalse(isNodeRed(root!!), "Корень должен быть черным")
