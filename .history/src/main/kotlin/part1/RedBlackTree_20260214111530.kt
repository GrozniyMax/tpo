...
class RedBlackTree<T : Comparable<T>> : MutableSet<T> {
    ...
    internal class Node<T>(var data: T) {
     ...
     */
    init {
        // Create a sentinel node that acts as NIL
    ...
    private fun createSentinel(): Node<T> {
        return Node(null as T).apply { 
            isRed = false
            left = this
            right = this
        }
    ...
    private fun fixInsert(node: Node<T>) {
        ...
        while (current.parent?.isRed == true) {
            if (current.parent === current.parent?.parent?.left) {
                ...
                if (uncle != sentinel && uncle?.isRed == true) { // Случай 1: дядя красный
                    uncle.isRed = false
    ...
    private fun minimum(node: Node<T>): Node<T> {
        var current = node
        while (current.left != sentinel && current.left != null) {
